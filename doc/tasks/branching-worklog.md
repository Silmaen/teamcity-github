...
| 2026-07-29 | Per-task cleanup opt-out | **No.** The levers are the server-wide `queueCleanup.enabled` switch and the presence of the build feature; no third per-configuration axis. | — |
...
| G1 | `pull_request.labeled` / `unlabeled` | **done (1.8.3)** | A label is now a trigger: the action re-evaluates the commit and enqueues what became eligible. Posts no skip row (see `PrAction.reportsSkips`). | `PrAction`, `handleInternal` |
| G3 | `pull_request.edited` | **done (1.8.3)** | Same path as G1: a title/body edit re-evaluates the metadata gate. | `PrAction`, `handleInternal` |
| G4 | `pull_request.reopened` | **done (1.8.3)** | Behaves like `opened`, skip rows included. | `PrAction` |
| G10 | Line-level Check Run annotations | **done (1.8.3)** | Compiler diagnostics (clang/gcc + MSVC) parsed from the build problems and emitted as `output.annotations`, capped at 50, paths relativised against the checkout dir. Test failures are not covered — TeamCity gives class/method, not file/line. | `BuildProblemAnnotations`, `CheckRunAnnotation`, `encodeCheckRunPayload` |
